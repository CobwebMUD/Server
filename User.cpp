...
void User::uListen() 
{
	...
	while (true)
	{
		...
		if (userInput == "2")
		{
			...
			// Break out of login loop since account has been generated
			break;
		}
		else if (userInput == "1") 
		{
			...
			if (account->loggedIn) 
			{
				...
				else 
				{
					...
					for (auto it = accountChars.begin(); it != accountChars.end(); ++it) 
					{
						...
						char promptTemp[strlen(prompt) + strlen(output.c_str())];
						strcpy(promptTemp, prompt);
						strcat(promptTemp, output.c_str());
						prompt = (const char*) promptTemp;
						write(clientID, prompt, strlen(prompt));
						// Recv character selection
						read(clientID, fromUser, (sizeof(fromUser) / sizeof(char)));
						if (strcmp(fromUser, "") == 0)
						{
							return;	
						}
						int ID = atoi((const char*) fromUser);
						delete character;
		...
		{
			const char* prompt = "Invalid input!\0";
			write(clientID, prompt, strlen(prompt));
			continue;
		}
	}
	if (loggedIn)
	{
		...
		inventory = new Inventory(std::string(account->details.name + character->getName()));
		while (true)
		{
			clearBuff(fromUser, (sizeof(fromUser) / sizeof(char)));
			// listen for intput from User
			memset(fromUser, 0, 128);
			read(clientID, fromUser, (sizeof(fromUser) / sizeof(char)));
			std::string userInput = std::string(fromUser);
			trimStr(userInput);
...
void User::startGame()
{
	...
	strcpy(socketP, "gameScripts/.sock");
	strcat(socketP, cClientID);
	// UD socket creation
	int sock = socket(AF_UNIX, SOCK_STREAM, 0);
	struct sockaddr_un addr;
	addr.sun_family = AF_UNIX;
	strncpy(addr.sun_path, socketP, sizeof(addr.sun_path) - 1);
	unlink(socketP);
	...
	while (true)
	{
		...
		// Read data from game script socket 
		...
		{	
			...
			{
				// Send the output from the game script socket to the user
				write(clientID, recv, strlen(recv));
				// Wait for response from user socket
				clearBuff(recv, rChunkS);
